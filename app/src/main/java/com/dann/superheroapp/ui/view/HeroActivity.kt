...
class HeroActivity : AppCompatActivity() {
    ...
    private fun getHerolInformation() {
        ...
        heroUrl = "${hero.thumbnail.path}/standard_fantastic.${hero.thumbnail.extension}".replace(
            "http",
            "https"
        )
        for (comic in hero.comics.items){
            val uri = "${comic.resourceURI}?ts=1&apikey=6297766f747707d8733f11ec846ba65c&hash=9cae473e2dbf1e5c71aed13ab674a620".replace("http","https")
            heroViewModel.onCreate(uri)
        }
    }

    private fun setInformationInViews() {
        if (heroUrl.split("/")[10] != "image_not_available") {
            Glide.with(this).load(heroUrl).centerCrop().into(binding.imgHeroDetail)
        } else {
    ...
    private fun refresh(){ if(!comicImageList.isNullOrEmpty()){ adapter.notifyDataSetChanged() } }
}